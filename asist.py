...
try:
    ...
    # Prueba de conexión
    test = supabase.table("estudiantes").select("ru").limit(1).execute()
...
with st.sidebar:
    ...
    # Botón de prueba de Supabase
    if st.button("🔌 Probar conexión a Supabase"):
        try:
            test = supabase.table("estudiantes").select("ru").limit(1).execute()
            st.success("✅ Conexión exitosa")
            st.info("Tabla 'estudiantes' accesible")
            # Probar inserción de asistencia con datos dummy
            try:
                dummy = supabase.table("asistencia").select("id").limit(1).execute()
                st.success("✅ Tabla 'asistencia' accesible")
            except Exception as e2:
                st.error(f"❌ Tabla 'asistencia' no accesible: {e2}")
        except Exception as e:
            st.error(f"❌ Error: {e}")
...
# ------------------------------------------------------------
# ESCANEAR QR — VERSIÓN DEFINITIVA CON DEBUG
# ------------------------------------------------------------
    ...
    if ru_from_js and ru_from_js != st.session_state.get("qr_ru_procesado"):
        ...
        with st.spinner(f"Procesando RU {ru_from_js}..."):
                ...
                if not tiene_registro:
                    try:
                        ...
                        data = {
                            ...
                            "fecha": fecha.isoformat(),  # Asegurar formato YYYY-MM-DD
                            "hora": str(hora),
                            "estado": "Presente"
                        }
                        # Intentar inserción
                        ...
                        if result.data:
                            ...
                            st.session_state.mensaje_escaneo = {
                                "tipo": "error",
                                "texto": "⚠️ La inserción no devolvió datos. Revisa la tabla."
                            }
                    except Exception as e:
...
<script>
  ...
  function onScanSuccess(decodedText) {
    ...
    statusDiv.textContent = '✅ QR detectado: ' + decodedText + ' → registrando...';
  ...
  html5QrCode.start({ facingMode: "environment" }, config, onScanSuccess, onScanError)
    ...
    .catch(err => {
      console.error(err);
      document.getElementById('status').textContent = '❌ Error al iniciar cámara: ' + err;
    });
